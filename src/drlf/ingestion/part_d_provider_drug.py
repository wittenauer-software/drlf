...
def _row_values(row: dict[str, Any], source_release_id: int, data_year: int) -> tuple[Any, ...]:
    return (
        ...
        data_year,
        str(row["Prscrbr_NPI"]),
        row.get("Prscrbr_Last_Org_Name"),
        row.get("Prscrbr_First_Name"),
        row.get("Prscrbr_City"),
        row.get("Prscrbr_State_Abrvtn"),
        row.get("Prscrbr_State_FIPS"),
        row.get("Prscrbr_Type"),
        row.get("Prscrbr_Type_Src", row.get("Prscrbr_Type_src")),
        row["Brnd_Name"],
        row["Gnrc_Name"],
        _integer(row.get("Tot_Clms")),
        _decimal(row.get("Tot_30day_Fills")),
        _integer(row.get("Tot_Day_Suply")),
        ...
        _integer(row.get("GE65_Tot_Day_Suply")),
...
def _validate_manifest_identity(manifest: dict[str, Any]) -> int:
    ...
    if dataset["dataset_id"] != DATASET_TYPE_UUID or dataset["slug"] != DATASET_SLUG:
        raise ValueError("Manifest is not for CMS Part D Prescribers by Provider and Drug")
    if dataset["aggregation_keys"] != list(AGGREGATION_KEYS):
        raise ValueError("Manifest aggregation keys do not match the provider-and-drug grain")
    data_year = dataset["data_year"]
    if not isinstance(data_year, int):
        raise ValueError("Part D provider/drug manifests require an integer data_year")
    return data_year


def load_part_d_provider_drug(
    database_url: str,
    manifest_path: Path,
    *,
    code_commit: str,
    ...
    """Load a complete filtered Part D provider/drug manifest into PostgreSQL."""
    ...
    with psycopg.connect(database_url) as connection, connection.transaction():
        ...
        connection.execute(
            "delete from claims.part_d_provider_drug where source_release_id = %s",
            (source_release_id,),
        )

        loaded_rows = 0
        columns = (
            "source_release_id, data_year, prescriber_npi, prescriber_last_org_name, "
            "prescriber_first_name, prescriber_city, prescriber_state, prescriber_state_fips, "
            "prescriber_type, prescriber_type_source, brand_name, generic_name, total_claims, "
            "total_30day_fills, total_day_supply, total_drug_cost, total_beneficiaries, "
            "ge65_suppression_flag, ge65_total_claims, ge65_total_30day_fills, "
            "ge65_total_day_supply, ge65_total_drug_cost, "
            "ge65_beneficiary_suppression_flag, ge65_total_beneficiaries"
        )
        with connection.cursor().copy(
            f"copy claims.part_d_provider_drug ({columns}) from stdin"  # noqa: S608
        ) as copy:
            for file_record in files:
