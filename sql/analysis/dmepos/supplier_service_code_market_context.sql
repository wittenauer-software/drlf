-- Aggregate market context for retained, exact-code DMEPOS supplier-service universes.
--
-- The output grain is year/HCPCS/rental indicator. All totals and shares use
-- visible published supplier-service cells only; suppressed beneficiary counts
-- remain in monetary and service totals but are excluded from per-beneficiary
-- distributions. Supplier state is a source address assertion, not beneficiary
-- residence or the furnishing location.
with settings as materialized (
    ...
    select case
        when valid then 1
            ...
            'DMEPOS code-market parameter validation failed; require one to three '
...
), market_quartiles as materialized (
    select
        data_year,
        ...
        supplier_rental_indicator,
        count(*) as total_visible_cells,
        count(*) filter (where known_beneficiary_flag)
            as known_beneficiary_cells,
        count(*) filter (
            where known_beneficiary_flag
              and not candidate_cohort_flag
        ) as cohort_excluded_known_beneficiary_peer_cells,
        count(*) filter (where not known_beneficiary_flag)
            as suppressed_or_unknown_beneficiary_cells,
        count(*) filter (where upper(supplier_state) = 'FL')
            as source_address_florida_visible_cells,
        count(*) filter (where candidate_cohort_flag)
            as candidate_cohort_visible_cells,
        sum(total_services) as total_visible_services,
        sum(reconstructed_medicare_payment_amount)
            as reconstructed_visible_medicare_payment_amount,
        sum(reconstructed_medicare_standardized_payment_amount)
            as reconstructed_visible_standardized_payment_amount,
        coalesce(sum(total_services) filter (
            where upper(supplier_state) = 'FL'
        ), 0) as source_address_florida_visible_services,
        coalesce(sum(reconstructed_medicare_payment_amount) filter (
            where upper(supplier_state) = 'FL'
        ), 0) as source_address_florida_visible_medicare_payment,
        coalesce(sum(reconstructed_medicare_standardized_payment_amount) filter (
            where upper(supplier_state) = 'FL'
        ), 0) as source_address_florida_visible_standardized_payment,
        coalesce(sum(total_services) filter (
            where candidate_cohort_flag
        ), 0) as candidate_cohort_visible_services,
        coalesce(sum(reconstructed_medicare_payment_amount) filter (
            where candidate_cohort_flag
        ), 0) as candidate_cohort_visible_medicare_payment,
        coalesce(sum(reconstructed_medicare_standardized_payment_amount) filter (
            where candidate_cohort_flag
        ), 0) as candidate_cohort_visible_standardized_payment,
        max(reconstructed_medicare_payment_amount)
            / nullif(sum(reconstructed_medicare_payment_amount), 0)
            as top_supplier_visible_payment_share,
        sum(
            ...
            * sum(reconstructed_medicare_payment_amount),
            0
        ) as visible_payment_hhi,
        (percentile_cont(0.50) within group (
            order by services_per_beneficiary
        ) filter (where known_beneficiary_flag))::numeric
            as known_beneficiary_services_median,
        (percentile_cont(0.75) within group (
            order by services_per_beneficiary
        ) filter (where known_beneficiary_flag))::numeric
            as known_beneficiary_services_q75,
        (percentile_cont(0.50) within group (
            order by medicare_payment_per_beneficiary
        ) filter (where known_beneficiary_flag))::numeric
            as known_beneficiary_medicare_payment_median,
        (percentile_cont(0.75) within group (
            order by medicare_payment_per_beneficiary
        ) filter (where known_beneficiary_flag))::numeric
            as known_beneficiary_medicare_payment_q75,
        (percentile_cont(0.50) within group (
            order by standardized_payment_per_beneficiary
        ) filter (where known_beneficiary_flag))::numeric
            as known_beneficiary_standardized_payment_median,
        (percentile_cont(0.75) within group (
            order by standardized_payment_per_beneficiary
        ) filter (where known_beneficiary_flag))::numeric
            as known_beneficiary_standardized_payment_q75
    ...
    select
        market.*,
        market.source_address_florida_visible_services
            / nullif(market.total_visible_services, 0)
            as source_address_florida_visible_service_share,
        market.source_address_florida_visible_medicare_payment
            / nullif(market.reconstructed_visible_medicare_payment_amount, 0)
            as source_address_florida_visible_payment_share,
        market.source_address_florida_visible_standardized_payment
            / nullif(market.reconstructed_visible_standardized_payment_amount, 0)
            as source_address_florida_visible_standardized_payment_share,
        market.candidate_cohort_visible_services
            / nullif(market.total_visible_services, 0)
            as candidate_cohort_visible_service_share,
        market.candidate_cohort_visible_medicare_payment
            / nullif(market.reconstructed_visible_medicare_payment_amount, 0)
            as candidate_cohort_visible_payment_share,
        market.candidate_cohort_visible_standardized_payment
            / nullif(market.reconstructed_visible_standardized_payment_amount, 0)
            as candidate_cohort_visible_standardized_payment_share,
        case
            when market.cohort_excluded_known_beneficiary_peer_cells
                >= settings.minimum_peer_count
                then 'scoreable'
            else 'insufficient-cohort-excluded-peer-cells-descriptive'
        end as known_beneficiary_peer_status
    from market_quartiles as market
    cross join settings
...
select
    ...
    output.total_visible_cells,
    output.known_beneficiary_cells,
    output.cohort_excluded_known_beneficiary_peer_cells,
    output.suppressed_or_unknown_beneficiary_cells,
    output.known_beneficiary_peer_status,
    output.total_visible_services,
    output.reconstructed_visible_medicare_payment_amount,
    output.reconstructed_visible_standardized_payment_amount,
    output.source_address_florida_visible_cells,
    output.source_address_florida_visible_services,
    output.source_address_florida_visible_medicare_payment,
    output.source_address_florida_visible_standardized_payment,
    ...
    output.candidate_cohort_visible_cells,
    output.candidate_cohort_visible_services,
    output.candidate_cohort_visible_medicare_payment,
    output.candidate_cohort_visible_standardized_payment,
    ...
    output.top_supplier_visible_payment_share,
    output.visible_payment_hhi,
    output.known_beneficiary_services_median,
    output.known_beneficiary_services_q75,
    output.known_beneficiary_medicare_payment_median,
    output.known_beneficiary_medicare_payment_q75,
    output.known_beneficiary_standardized_payment_median,
    output.known_beneficiary_standardized_payment_q75,
    settings.minimum_peer_count,
